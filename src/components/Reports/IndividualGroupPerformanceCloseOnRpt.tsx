...
const IndividualGroupPerformanceCloseOnRpt: React.FC = () => {
  const { user } = useAuth();
  ...
  const [payoutUpdateError, setPayoutUpdateError] = useState<string | null>(null);
  ...
  const handleTileClick = useCallback(
    (statusValue: string) => {
      ...
      fetchCommPayouts(searchTerm, statusValue, startDate, endDate);
    },
    [fetchCommPayouts, searchTerm, startDate, endDate]
  );
  ...
  const tableRows: CommRow[] = useMemo(() => {
    ...
    filteredProperties.forEach((p) => {
      ...
      const submissionDate = formatDate(
        anyP.SubmissionDate || (p as any).AddDate
      );
      ...
      const introducerFee = toNumber(
        anyP.IntroCommAmt ?? anyP.IntroCommAmt
      );
  ...
  const viewRows = useMemo(() => {
    if (renFilter === 'all') return tableRows;
    ...
    try {
      ...
      setProperties((prev: any[]) =>
        prev.map((p) => {
          ...
          const pTxId =
            String(anyP.TxId || anyP.TXID || anyP.RefNo || anyP.ApplicationId || '');
  ...
  const handleBulkUpdatePayoutDate = async () => {
    ...
    const payloadRows: { ApplicationId: number; TransId: string; PayoutDate: string }[] = [];
      ...
      if (result?.status === 'success' || result?.status === 'partial') {
        ...
        setProperties((prev: any[]) =>
          prev.map((p) => {
            ...
            const pTransId = String(anyP.TransId || anyP.transId || anyP.TRANSID || '').trim();
            const pTransId1 = String(anyP.TransId1 || anyP.transId1 || anyP.TRANSID1 || '').trim();
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            ...
            Individual Group performance (Close On)
          </h1>
        </div>
      </div>
      ...
      {!loading && !error && rawData && rawData.length > 0 && (
        <div className="bg-white rounded-xl shadow-lg p-4">          
          {false && (
            <pre className="text-xs overflow-auto max-h-[400px] bg-gray-50 p-3 rounded-lg">
            ...
            <table className="min-w-full text-xs md:text-sm border-collapse">
              <thead>
                <tr className="bg-gray-100 text-gray-700">
                  ...
                  <th className="px-3 py-2 text-right border-b">Less Introducer Fees</th>
                ...
                {viewRows.map((row, idx) => {
                  ...
                  return (
                    <tr key={`${row.kind}-${idx}`} className={rowClass}>
                      <td className={baseClass}>
                        <input
                          ...
                          onBlur={(e) => {
                            ...
                            setPayoutInputMap((prev) => ({ ...prev, [k]: formatted }));
                          }}
                        />
                      </td>
                      ...
                      <td className={`${baseClass} text-right`}>{formatCurrency(row.introducerFee)}</td>
                      <td className={`${baseClass} text-right`}>{formatCurrency(row.netFees)}</td>
                      <td className={`${baseClass} text-right`}>{formatCurrency(row.commission)}</td>
                      <td className={`${baseClass} text-right`}>{formatCurrency(row.trainingFees)}</td>

                      <td className={`${baseClass} text-right`}>
                        {formatCurrency(toNumber(row.commission) + toNumber(row.trainingFees))}
                      </td>

                      <td className={`${baseClass} text-right`}>{formatCurrency(row.withholdingTaxAmt)}</td>
                      <td className={`${baseClass} text-right`}>{formatCurrency(row.adminCharges)}</td>

                      <td className={`${baseClass} text-right`}>{formatCurrency(row.stampDuty)}</td>
                      <td className={`${baseClass} text-right`}>{formatCurrency(row.netComm)}</td>
...
export default IndividualGroupPerformanceCloseOnRpt;