...
interface TenantEdInvoicePopupProps {
  isOpen: boolean;
  onClose: () => void;
  party: PreviewParty;
...
const TenantEdInvoicePopup: React.FC<TenantEdInvoicePopupProps> = ({
  isOpen,
  onClose,
  party,
  paymentTransId,
  ...
  useEffect(() => {
    ...
    const fallback =
      ...
      ` ED Invoice for PaymentTransId ${paymentTransId}`;
  ...
  const generateInvoiceNow = async (invoiceParty: PreviewParty) => {
    try {
      ...
      alert(
        json.data || `${isTenant ? tenantLabel : landlordLabel} ED invoice generated successfully.`
      );
      onSuccess?.();
    } catch (e: any) {
      console.error("ED invoice generation failed:", e);
      alert(e?.message || "Failed to generate ED invoice.");
    }
  };
    ...
    >
      <div className="w-full max-w-4xl bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="px-4 py-3 border-b flex items-center justify-between bg-gray-100">
          <div className="font-semibold">ED Invoice</div>
          <button
            type="button"
            onClick={handleClose}
        ...
        <div className="p-4 space-y-4 text-sm">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
            <div className="md:col-span-2 font-semibold text-gray-700">ED Invoice No:</div>
          ...
          <div className="pt-2 flex items-center justify-end gap-2">
            ...
            >
              {generatingInvoice ? "Generating..." : "Generate ED Invoice"}
            </button>
          </div>
        </div>
      </div>
...
export default TenantEdInvoicePopup;