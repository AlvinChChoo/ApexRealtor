...
const buildDocDefinitionTal = (title: string, logoDataUrl?: string, footerDataUrl?: string, _ctx?: { refNo?: string; docMLogId?: string }) => ({
  info: { title, author: 'InteRealtor', subject: 'Tenant Appointing Letter', keywords: 'TAL, InteRealtor' },
  pageSize: 'A4',
  pageMargins: [40, 80, 40, 85],
      ...
      stack: [
        {
          columns: [
            ...
            { text: '', width: '*' },
            {
              //width: '',
              alignment: 'right',
              table: { widths: ['auto'], body: [[{ text: 'TENANT APPOINTING LETTER', color: '#fff', bold: true, alignment: 'center', fontSize: 10, margin: [20,6,20,6], fillColor: '#000' }]] },
              layout: 'noBorders',
            },
...
*/
  ...
  styles: {
    terms: { fontSize: 9, lineHeight: 1.2 },
  ...
  content: [
    ...
    oneRowText(''),
    oneRowText('Dear Sir,'),
    oneRowText('<b>RE: APPOINTMENT OF INTEREALTOR TO SECURE A PROPERTY FOR RENT</b>'),
    oneRowText(''),
    oneRowText(
      `I/We 1) <u>${__wtaApplication?.Tenant1Name || ''}</u>          Passport / Nric No. <u>${__wtaApplication?.Tenant1Id || ''}</u>`
    ),
    blankDivider(' ',0.5),
    oneRowText('do hereby appoint InteRealtor to secure a property for RENTAL, subject to the following requirements:'),
    blankDivider(' ',0.5),
    oneRowText(
      `a) The rental for the said property shall not exceed <u>RM ${__wtaApplication?.AtlRentalAmt ?? '0.00'}</u> per month.`
    ),
    oneRowText(
      `b) Type of the property preferred Condominium location <u>${__wtaApplication?.PropertyLocation ?? ''}</u>.`
    ),
    blankDivider(' ',0.5),
    ...
    blankDivider(' ',0.5),
    oneRowText('Further, I have no objection in InteRealtor\'s Negotiator making Additional Claims for expenses incurred in rendering of your value added services as in accordance to the Act 1981 (Act 242) & Rules under the Seventh Schedule C4, provided always such claim is reasonable and fair.'),
    blankDivider(' ',0.5),
    oneRowText(
      `Hence, I am agreeable to pay <u>RM ${(((parseFloat(__wtaApplication?.TalServiceFeesAmt ?? '0') || 0) + (parseFloat(__wtaApplication?.TalProfessionalFeesSstAmt ?? '0') || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }))}</u> inclusive 8% SST upon signing the Tenancy Agreement directly to InteRealtor Sdn Bhd (Clients Acc - PBB 32 1707 6233).`
    ),
    blankDivider(' ',0.5),
    oneRowText('This letter is given to InteRealtor in my capacity as the Tenant...'),
    contentRow('',''),
    {  
      margin: [120, 5, 120, 5],
      table: {
            ...
            table: {
              ...
              body: [
                [            
                  ...
                  { text: `RM ${(parseFloat(__wtaApplication?.TalServiceFeesAmt ?? '0') || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, alignment: 'right', style: 'terms' },
                ],
                [
                  { text: 'SST (8%)', style: 'terms'},
                  { text: `RM ${(parseFloat(__wtaApplication?.TalProfessionalFeesSstAmt ?? '0') || 0).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, style: 'terms', alignment: 'right', border: [false, false, false, true] }
                ],
                [
                  { text: 'Total', style: 'terms'},
                  {
                    text: `RM ${(((parseFloat(__wtaApplication?.TalServiceFeesAmt ?? '0') || 0) + (parseFloat(__wtaApplication?.TalProfessionalFeesSstAmt ?? '0') || 0)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }))}`,                   
                    style: 'terms',
                    alignment: 'right',
    ...
    signatureRow('REN / NAME' ,'','',''),     
    signatureRow('REN NO / NRIC','','',''),  
    signatureRow('DATE','','',''),                    
  ],
} as any);
...
const buildDocDefinitionWta = (title: string, logoDataUrl?: string, footerDataUrl?: string , ctx?: { refNo?: string; docMLogId?: string }) => ({
  ...
  content: [
    ...
    oneRowText(''),  
    oneRowText('I/We the undersigned Tenant(s) do hereby engage <b>INTEREALTOR SDN BHD</b> a real estate firm registered with the Board of Valuer, Appraisers, Estate Agents, and Property Managers, to assist My/Our renting process.'),
    oneRowText('') ,
    oneRowText('Services and assistance I/We may require from InteRealtor including but not limited to the following:'),
    oneRowText(' - To provide Comparative Market Analysis on the rental market of the subject property.'),
    oneRowText(' - To coordinate in signing of the Tenancy Agreement (TA).'),
    oneRowText(' - To brief Me/Us on the terms of the Tenancy Agreement (TA).'),
    oneRowText(' - To collect keys and take Vacant Possession upon request.'),
    oneRowText(' - To confirm items included in the inventory list.'),
    oneRowText(' - To assist in sourcing for logistic and mover.'),
    oneRowText(' - To provide neighbourhood orientation.'),
    oneRowText(' - To assist in utilities applications.'),
    oneRowText(' - To assist in buying of additional household items.'),
    oneRowText(' - Sourcing & procuring of furniture, curtains, etc.'),
    oneRowText(' - To assist in sourcing for cleaning service.'),
    oneRowText(' - To provide periodical property inspection to ensure livable conditions.'),
    oneRowText(' - To send reminder on overdue rental.'),
    oneRowText(' - To convey complaints/feedback to the Landlord(s).'),
    oneRowText(' - To provide solution for defect management.'),
    oneRowText(' - To mitigate defect liability with the Landlord(s) at the end of Tenancy.'),
    oneRowText(' - To assist in the refund of all the deposits upon expiry of the Tenancy.'),
    oneRowText(' - To assist in rent review for the optional period.'),
    oneRowText(` - Others : <u>${__wtaApplication?.WtatExtraServices ?? ''}</u>`),
    oneRowText(''),
    oneRowText('I/We are fully aware that you are merely providing your value-added services upon request.'),
    oneRowText(''),
    oneRowText(
  `I/We hereby agree to pay you a service fees of <u>RM ${(
    (parseFloat(__wtaApplication?.TalServiceFeesAmt ?? 0) || 0) +
    (parseFloat(__wtaApplication?.TalProfessionalFeesSstAmt ?? 0) || 0)
  ).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</u> inclusive 8% SST to <b>INTEREALTOR SDN BHD</b>`
),
    oneRowText(''),
  ...
  body: [
    [            
      ...
      { 
        text: `RM ${(parseFloat(__wtaApplication?.TalServiceFeesAmt ?? 0) || 0).toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`,
        alignment: 'right',
    ...
    [
      { text: 'SST (8%)', style: 'terms' },
      { 
        text: `RM ${(parseFloat(__wtaApplication?.TalProfessionalFeesSstAmt ?? 0) || 0).toLocaleString(undefined, {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`,
        style: 'terms',
...
const buildDocDefinitionGATL = (title: string, logoDataUrl?: string, footerDataUrl?: string, _ctx?: { refNo?: string; docMLogId?: string }) =>
  ({
    ...
    content: [
      ...
      oneRowText('I/we, the undersigned Tenant(s) hereby confirm our intention to rent the Property at a monthly rental of RM 1100 subject to the following terms and conditions: -'),
      oneRowText(''),
...
clause 8 herein) shall be fully forfeited by the Landlord(s) as liquidated damages.`),
      ...
      signatureRow('SIGNED ON','10-Jan-2023','10-Jan-2023',''),
      contentRow('',''),
      contentRow('',''),
      contentRow('',''),
      contentRow('',''),
...
async function runFullFlowGATL(opts: {
  ...
  await uploadPdfBlob(blob, appId, newDocId, filePrefix, uploadUrl, uploadProxyUrl);
  return blob;
}

export interface WTASectionProps {
...
const WTASection: React.FC<WTASectionProps> = ({
  application,
  applicationId,
  defaultFormType = 'TAL',
  defaultSSTRate = 0.08,
  ...
  // per-doc config
  talDocMUid = '81F469DC-3EDA-4F9C-AD26-1FAFF254F253',
  talDocContent1 = 'TENANT APPOINTING LETTER (TAL)',
  ...
  // 🔒 Case-insensitive edit gate (NO UI changes)
  const status = String(application?.ApplicationStatus ?? '').toLowerCase().trim();
const canEdit = ['active', 'pending review', 'rejected'].includes(status);
  ...
  const generateTal = async () => {
    ...
    try {
      ...
      const blob = await runFullFlowGATL({
        title: 'TENANT APPOINTING LETTER (TAL)',
        appId: resolvedAppId,
        docUid: talDocMUid,
        ...
        uploadUrl,
        uploadProxyUrl,
        buildDocDefinition: buildDocDefinitionTal,
        filePrefix: 'TAL',
      });
      ...
      alert(e?.message || 'Failed to generate TAL.');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const generateWta = async () => {
    ...
    try {
      ...
      const blob = await runFullFlowGATL({
        ...
        uploadUrl,
        uploadProxyUrl,
        buildDocDefinition: buildDocDefinitionWta,
        filePrefix: 'WTA',
      });
      const url = URL.createObjectURL(blob);
      setPdfUrl(url);
      ...
      <div className="bg-white rounded-xl shadow-lg overflow-hidden mb-6">
        ...
        >
          <h2 className="text-lg font-semibold">
            WARRANT TO ACT / TENANT APPOINTING LETTER (WTAT)
          </h2>
            ...
            <div className="grid grid-cols-12 gap-3 mb-2 items-center">
              ...
              <div className="col-span-10">
                <select
                  ...
                  disabled={!canEdit}
                >
                  <option value="TAL">TENANT APPOINTING LETTER - TAL</option>
            ...
            <div className="flex justify-center space-x-4">
              <button
                ...
                className={`px-6 py-2 rounded-lg text-white font-medium ${
                  saving || !canEdit ? 'bg-green-600/60 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'
                }`}
              >
                {saving ? 'Saving…' : (isTAL ? 'UPDATE TAL DETAILS' : 'UPDATE WTA DETAILS')}
              </button>
              ...
              <button
                ...
                className={`text-white px-6 py-2 rounded-lg transition-colors font-medium ${
                  generating || !canEdit ? 'bg-red-600/60 cursor-not-allowed' : 'bg-red-600 hover:bg-red-700'
                }`}
              >
                {generating ? 'Generating…' : `GENERATE ${isTAL ? 'TAL' : 'WTA'}`}
              </button>
            </div>
          </div>
        )}
