...
// src/components/Payments/BuyerProformaInvoicePopup.tsx
...
interface BuyerProformaInvoicePopupProps {
  isOpen: boolean;
  onClose: () => void;
  party: PreviewParty;
...
const BuyerProformaInvoicePopup: React.FC<BuyerProformaInvoicePopupProps> = ({
  isOpen,
  onClose,
  party,
  paymentTransId,
  ...
  application,
  tenantServiceFees,
  tenantAdminChargesTotal, // kept for props compatibility
  landlordAdminChargesTotal, // kept for props compatibility
  tenantStampDuty,
  landlordStampDuty,
  tenantToRen,
  landlordToRen,
  ...
  // ✅ NEW: editable description state (textarea will bind to this)
  const [descriptionText, setDescriptionText] = useState<string>("");

  // ✅ NEW: To / Address fields (between Invoice No and Description)
...
useEffect(() => {
  ...
  const filteredOptions = useMemo(() => {
    ...
    return descRows.filter((r) => allowed.has(String(r.DescGroup || "").toUpperCase()));
  }, [descRows]);

  const selectedDescText = useMemo(() => {
    const row = filteredOptions.find((x) => String(x.DescId) === String(selectedDescId));
    return row?.DescText ?? "";
  ...
  const parts = [
    String(app?.PropertyAddress ?? "").trim(),
    String(app?.PropertyPostalCode ?? "").trim(),
    String(app?.PropertyState ?? "").trim(),
  ].filter(Boolean);

  const addrFull = parts.join(", ");
  if (!addrFull) return text;

  // avoid double add (basic check)
  if (text.toLowerCase().includes(addrFull.toLowerCase())) return text;

  const base = text.trim();
  const sep = base.endsWith(":") ? " " : ": ";
  return `${base}${sep}${addrFull}`;
};
  */
...
const appendAddress = (text: string) => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        // auto-select first matching group
  ...
  }, [isOpen, selectedDescText, party, tenantLabel, landlordLabel, paymentTransId, application]);

  // ✅ preview build (invoice calc)
  useEffect(() => {
    if (!isOpen) return;
    ...
    // Fees (TENANT vs LANDLORD)
    const feesGross = isTenant ? toNum(tenantServiceFees) : toNum((application as any)?.AtlProFees);
    ...
    // Invoice amount includes Admin SST too
  ...
  const generateInvoiceNow = async (invoiceParty: PreviewParty) => {
    try {
      ...
      // ✅ keep backend naming stable (same as your original component)
      ...
      alert(json.data || `${isTenant ? tenantLabel : landlordLabel} invoice generated successfully.`);
      onSuccess?.();
    } catch (e: any) {
...
/>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
            ...
            <div className="md:col-span-2 font-semibold text-gray-700 text-center">+8% SST:</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
            ...
            <div className="md:col-span-2 font-semibold text-gray-700 text-right">Total (RM) =</div>
            <div className="md:col-span-3">
              <input
                className="w-full border rounded px-3 py-2 bg-gray-100 text-right font-semibold"
                readOnly
...
export default BuyerProformaInvoicePopup;
