...
const FormSession: React.FC<FormSessionProps> = ({ applicationId }) => {
  const [rows, setRows] = useState<FormRow[]>([]);
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState<string | null>(null);
  const [lastFetchedAt, setLastFetchedAt] = useState<string | null>(null);
  const [showHistoryModal, setShowHistoryModal] = useState(false);
  const [selectedForm, setSelectedForm] = useState<{ docMUid: string; docDesc: string } | null>(null);
  ...
  const fetchForms = async () => {
    ...
    try {
      ...
      body.append('DocGroup', 'RENTAL'); // fixed to RENTAL
  ...
  const handleViewClick = (form: FormRow) => {
    setSelectedForm({ docMUid: form.DocMUid, docDesc: form.DocDesc });
    ...
    <>
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-gray-800">Rental Agreement Forms</h3>
          <button
            onClick={fetchForms}
        ...
        <div className="text-xs text-gray-500 mb-3">
          DocGroup used: <span className="font-medium">RENTAL</span>
