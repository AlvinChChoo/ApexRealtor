...
const INT_INVOICE_GET_URL = API_ENDPOINTS.INT_INVOICE_GET;

export const printInvoicePdf = async (bundle: IntInvoiceBundle) => {
  ...
  const rawInvoiceDate =
    ...
    inv.InvoiceDate ??
    (inv as any)?.Invoice_Date ??
    "";

  const invoiceDate = formatDate(rawInvoiceDate);

  const invoiceType = invoiceApi?.InvoiceType ?? inv.InvoiceType ?? "Tenant";
  const terms = (inv as any)?.Terms ?? (inv as any)?.PaymentTerms ?? "CASH";

  const invoiceDescRaw = String(
  ...
  const invoiceDescLines = invoiceDescRaw
    .replace(/\r\n/g, "\n")
    .split("\n")
    .map((s: string) => s.trim())
    .filter(Boolean);

  const headerTo = String(invoiceApi?.InvoiceTo ?? "").trim();
  const headerAddress = String(invoiceApi?.InvoiceAddress ?? "").trim();
  ...
  const renName = String(
    invoiceApi?.RenName ?? (inv as any)?.RenName ?? ""
  ).trim();
  const renRegNo = String(
  ...
  const [logoDataUrl, footerDataUrl] = await Promise.all([
    ...
    assetToDataUrl(footerPng).catch((err) => {
      console.error("Failed to load footer (import):", err);
      return "";
    }),
  ]);
  ...
  const docDefinition: any = {
    ...
    footer: (currentPage: number, pageCount: number, pageSize: any) => {
      ...
      const banner = footerDataUrl
        ...
        : { text: "Footer image not loaded", fontSize: 8, color: "red" };
  ...
  columns: [
    ...
    {
      width: 185,
      canvas: [{ type: "line", x1: 0, y1: 0, x2: 185, y2: 0, lineWidth: 1 }],
    },
  ],
  margin: [0, 0, 0, 5],
},
  ...
  columns: [
    ...
    {
      width: 185,
      canvas: [{ type: "line", x1: 0, y1: 0, x2: 185, y2: 0, lineWidth: 2 }],
    },
  ],
  margin: [0, 2, 0, 0],
},
